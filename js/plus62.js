...
(function ($, document) {
  ...
  var $document = $(document),
    PRODUCTS_KEY = "plus62-items",
    $wItemsCount = $('[data-plus62-items-count]'),
    instance = null,
    defaults = {
      debug: false,
  ...
  function submitForm(e) {
    ...
    var $form = $(e.currentTarget),
      $submit = $form.find('[data-plus62-submit]'),
      $item = $form.find('[data-plus62-item-id]');
    ...
    $form.trigger('plus62-item-'+action);

    // Show some debug information if the variable is set and evaluates to true
    if ( cf.defaults.debug ) {
      console.info("[PlusEnamDua] - Basket updated!");
      console.info("[PlusEnamDua] - Item " + action + "!");
    }
  }

  function serializeObject($form){
    var obj = {},
      arraySerial = $form.find('[data-plus62-attribute]').serializeArray();
  ...
  /**
   * Update counter items. Wrapper of the count is referenced by metadata 'data-plus62-items-count'
   * @return void
   */
  function updateCount(){

    var $itemsCount = $wItemsCount.find( '.plus62__num-items' );
    ...
    $.each($wItemsCount, function( index, value ){
      var $itemsCount = $(this).find('.plus62__num-items');

      if(!$itemsCount.length){
        $itemsCount = $( "<span class=\"plus62__num-items\" />" );
  ...
  function removeItem(id){
    ...
    if (id in items){
      ...
      // Show some debug information if the variable is set and evaluates to true
      if ( cf.defaults.debug ) {
        console.info("[PlusEnamDua] - Item not found in the basket.");
      }
    ...
    $document.trigger('plus62-item-deleted');

    // Show some debug information if the variable is set and evaluates to true
    if ( cf.defaults.debug ) {
      console.info("[PlusEnamDua] - Item deleted!" );
    }
  ...
  function clearBasket(){
    ...
    if($.jStorage.flush()){
      ...
      if ( cf.defaults.debug ) {
        console.info("[PlusEnamDua] - Empty basket!");
      }

      $document.trigger('plus62-clear-basket');

      return true;
    }

    if ( cf.defaults.debug ) {
      console.warn("[PlusEnamDua] - Problem when emptying basket!");
    }
  ...
  $.extend( PlusEnamDua.prototype, {
    init: function() {

      var cf = $.PlusEnamDua();

      if ( cf.defaults.debug ) {
        console.info("[PlusEnamDua] - Init");
      }

      updateCount();

      $document.on('submit', 'form[data-plus62-form]', submitForm);

      $document.on('click', '[data-plus62-clear-basket]', function(e) {
        e.preventDefault();
        clearBasket();
      });

      $document.on('click', '[data-plus62-delete-item]', function(e) {
        e.preventDefault();

        var productId = $(this).data('plus62-delete-item');
  ...
  $.extend( {
    PlusEnamDua: function( options ) {
      return ( new PlusEnamDua( options ) );
    }
  } );
